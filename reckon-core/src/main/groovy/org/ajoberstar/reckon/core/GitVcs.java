...
public class GitVcs implements Vcs {
  ...
  public Optional<Version> getCurrentVersion() {
    return Optional.of(getBaseVersion()).filter(BaseVersion::isAnyHead).map(BaseVersion::getAny);
  }
  ...
  private BaseVersion getBaseVersion() {
    try (RevWalk walk = new RevWalk(repo)) {
      ...
      for (Ref ref : repo.getRefDatabase().getRefs(Constants.R_TAGS).values()) {
        ...
        Optional<GitVersion> maybeVersion =
            tagParser.apply(tag).map(version -> new GitVersion(version, revCommit));
      ...
      GitVersion normal =
          findBase(
              walk,
              headCommit,
              versions.stream().filter(ver -> ver.getVersion().getPreReleaseVersion().isEmpty()));
      return new BaseVersion(
          normal.getVersion(), any.getVersion(), headCommit.equals(any.getRevCommit()));
    } catch (IOException e) {
  ...
  private GitVersion findBase(RevWalk walk, RevCommit head, Stream<GitVersion> versions) {
    try {
      ...
      Map<RevCommit, List<GitVersion>> versionsByRev =
          versions.collect(Collectors.groupingBy(GitVersion::getRevCommit));
