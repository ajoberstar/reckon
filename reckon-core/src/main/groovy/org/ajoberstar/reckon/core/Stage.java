...
public final class Stage implements Comparable<Stage> {
   ...
   * @param versioner the versioner used to infer the current version
   */
  ...
  /**
   * Comparison is performed solely based on the name. Names are ordered by normal lexicographical
   * order, with the exception of "final" being considered the highest precedence.
   *
  ...
  /**
   * Final stage will use only the normal version from the base and remove all other information.
   *
  ...
  /**
   * Fixed stages will always use pre-release information of the form {@code <stage
   * name>.<incremented count>}, where the count is always 1, unless the previous version used the
   * same stage. In that case the count will be incremented from the previous.
   *
   * @param name the name of the stage
  ...
  /**
   * Floating stages will behave as a fixed stage if the previous versions stage either doesn't
   * exist or has a lower precedence. If the previous has a higher precedence, the pre-release
   * information will be of the form {@code <previous stage>.<previous count>.<stage
   * name>.<incremented count>}.
   *
   * @param name the name of the stage
    ...
    return new Stage(
        ...
        (base, vcs) -> {
          ...
          if (Arrays.binarySearch(prevStages, name) >= 0) {
            ...
            return sanitized
                .setPreReleaseVersion(sanitized.getPreReleaseVersion() + "." + name)
